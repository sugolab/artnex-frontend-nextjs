'use client';

import { useState } from 'react';
import Link from 'next/link';
import { Button, Logo } from '@/components/common';
import { GNB, Footer } from '@/components/layout';

export default function RegisterPage() {
  const [isLoading, setIsLoading] = useState(false);

  const handleStartRegistration = async () => {
    setIsLoading(true);
    try {
      // 회원가입 시작 로직
      await new Promise(resolve => setTimeout(resolve, 1000));
      // Join_01로 이동
      window.location.href = '/register/step/1';
    } catch (error) {
      console.error('Registration start error:', error);
    } finally {
      setIsLoading(false);
    }
  };

  const handleGoogleSignup = () => {
    console.log('Google signup clicked');
    // Google OAuth 로직
  };

  const handleKakaoSignup = () => {
    console.log('Kakao signup clicked');
    // Kakao OAuth 로직
  };

  return (
    <div className="min-h-screen bg-white flex flex-col">
      <GNB />

      <div className="flex-1 flex items-center justify-center px-5">
        <div className="flex flex-col items-center text-center">
          {/* Logo */}
          <div className="mb-12">
            <Logo size="large" />
          </div>

          {/* Main Title - 피그마의 "Building your AI Manager" */}
          <h1 className="text-4xl font-noto-bold text-black mb-4">
            Building your AI Manager
          </h1>
          <p className="text-lg font-noto-medium text-gray-600 mb-12 max-w-md">
            브랜드 매니징을 위해 정보를 수집합니다.<br />
            누구나 쉽게 멋진 브랜딩을 할 수 있습니다.
          </p>

          {/* Action Buttons - 피그마 기준 3개 Buttons */}
          <div className="space-y-4">
            {/* 회원가입 버튼 */}
            <Button
              size="medium"
              loading={isLoading}
              onClick={handleStartRegistration}
            >
              회원가입하고 시작하기
            </Button>

            {/* Google 회원가입 */}
            <Button
              variant="secondary"
              onClick={handleGoogleSignup}
              className="flex items-center justify-center gap-3"
            >
              <svg width="20" height="20" viewBox="0 0 24 24">
                <path fill="#4285f4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                <path fill="#34a853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                <path fill="#fbbc05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                <path fill="#ea4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
              </svg>
              Google로 회원가입
            </Button>

            {/* Kakao 회원가입 */}
            <Button
              variant="secondary"
              onClick={handleKakaoSignup}
              className="bg-[#FEE500] border-[#FEE500] text-black hover:bg-[#FDD835] flex items-center justify-center gap-3"
            >
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 3C7.03 3 3 6.44 3 10.61c0 2.68 1.74 5.01 4.38 6.32l-.82 3.01c-.06.22.04.45.22.57.08.05.17.07.26.07.12 0 .23-.04.32-.12L10.4 17.5c.52.08 1.06.11 1.6.11 4.97 0 9-3.44 9-7.61S16.97 3 12 3"/>
              </svg>
              Kakao로 회원가입
            </Button>
          </div>

          {/* Login Link */}
          <div className="mt-8">
            <p className="text-sm font-noto-medium text-gray-600">
              이미 계정이 있으신가요?{' '}
              <Link 
                href="/login" 
                className="text-black hover:underline font-noto-bold"
              >
                로그인
              </Link>
            </p>
          </div>
        </div>
      </div>

      <Footer />
    </div>
  );
}